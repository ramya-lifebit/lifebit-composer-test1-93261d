...
summary['Output dir']                     = params.lifebitai_gwas_vcf_regenie_1.outdir
summary['Launch dir']                     = workflow.launchDir
summary['Working dir']                    = workflow.workDir
summary['Script dir']                     = workflow.projectDir
summary['User']                           = workflow.userName
summary['phenotype_colname']              = params.lifebitai_gwas_vcf_regenie_1.phenotype_colname
...
summary['number_of_files_to_process']     = params.lifebitai_gwas_vcf_regenie_1.number_of_files_to_process

summary['q_filter']                       = params.lifebitai_gwas_vcf_regenie_1.q_filter
summary['miss_test_p_threshold']          = params.lifebitai_gwas_vcf_regenie_1.miss_test_p_threshold
summary['variant_missingness']            = params.lifebitai_gwas_vcf_regenie_1.miss
summary['variant_minor_allele_freq']      = params.lifebitai_gwas_vcf_regenie_1.maf
summary['variant_minor_allele_count']     = params.lifebitai_gwas_vcf_regenie_1.mac
summary['hwe_threshold']                  = params.hwe_threshold
summary['extracted pruned region']        = params.lifebitai_gwas_vcf_regenie_1.extracted_prune_region

summary['remove_related_samples']         = params.lifebitai_gwas_vcf_regenie_1.remove_related_samples
summary['king_coefficient']               = params.lifebitai_gwas_vcf_regenie_1.king_coefficient
summary['king_plink_memory']              = params.lifebitai_gwas_vcf_regenie_1.king_plink_memory

summary['king_reference_data']            = params.king_reference_data
summary['run_ancestry_inference']         = params.lifebitai_gwas_vcf_regenie_1.run_ancestry_inference
summary['min_subpop_size']                = params.lifebitai_gwas_vcf_regenie_1.min_subpop_size

summary['run_pca']                        = params.lifebitai_gwas_vcf_regenie_1.run_pca
summary['number_pcs']                     = params.lifebitai_gwas_vcf_regenie_1.number_pcs
summary['remove_outliers_maxiter']           = params.lifebitai_gwas_vcf_regenie_1.remove_outliers_maxiter
summary['remove_outliers_sigma']             = params.lifebitai_gwas_vcf_regenie_1.remove_outliers_sigma


summary['gwas_cat']                       = params.gwas_cat

summary['outdir']                         = params.lifebitai_gwas_vcf_regenie_1.outdir
...
process standardise_phenofile_and_get_samples {
  ...
  script:
  """
  # make dummy transform file to move params.lifebitai_gwas_vcf_regenie_1.phenotype_colname to 3rd column
  # and IGNORE anything that is not the phenotype column or specified covariate_column

  if [ "${params.lifebitai_gwas_vcf_regenie_1.phenotype_colname}" = "false" ]; then
    pheno_col=\$(head -n 1 original.pheno.tsv | cut -f3 )
  else
    pheno_col=${params.lifebitai_gwas_vcf_regenie_1.phenotype_colname}
  fi
  ...
  lifebitai-gwas-vcf-regenie-1-transform_pheno.R \
...
process vcf2plink {
  tag "$name"
  label 'high_memory'
  publishDir "${params.lifebitai_gwas_vcf_regenie_1.outdir}/gwas_filtering", mode: 'copy'
  ...
  bcftools view -q ${params.lifebitai_gwas_vcf_regenie_1.q_filter} ${name}_downsampled.vcf.gz -Oz -o ${name}_filtered.vcf.gz
...
process filter_genotypic_data {
  tag "$name"
  publishDir "${params.lifebitai_gwas_vcf_regenie_1.outdir}/filter_genotypic_data", mode: 'copy'
  input:
  ...
  plink \
    ...
    --allow-no-sex \
    --mind ${params.lifebitai_gwas_vcf_regenie_1.mind_threshold} \
    --geno ${params.lifebitai_gwas_vcf_regenie_1.miss} \
    --maf ${params.lifebitai_gwas_vcf_regenie_1.maf} \
    --mac ${params.lifebitai_gwas_vcf_regenie_1.mac} \
    --memory ${plink_memory} \
    --1 \
    --keep-allele-order \
    --make-bed \
...
process merge_plink {
  publishDir "${params.lifebitai_gwas_vcf_regenie_1.outdir}/merged_plink", mode: 'copy'
...
process check_sample_sex {
  tag "Sex check"
  publishDir "${params.lifebitai_gwas_vcf_regenie_1.outdir}/sex_check", mode: 'copy'
...
process ld_prune {
  publishDir "${params.lifebitai_gwas_vcf_regenie_1.outdir}/merged_pruned_plink", mode: 'copy'
  ...
  script:
  plink_memory = extractInt(task.memory.toString()) * 1000
  extract_options = params.lifebitai_gwas_vcf_regenie_1.extracted_prune_region ? "--extract merged.prune.in" : ""
  """
  plink \
  --bfile merged \
  --keep-allele-order \
...
process calculate_relatedness {
    ...
    script:
    plink_memory = params.lifebitai_gwas_vcf_regenie_1.king_plink_memory ? "--memory ${params.lifebitai_gwas_vcf_regenie_1.king_plink_memory}" : ""
    """
    plink2 \
        --bfile ${bim.baseName} \
        --king-cutoff ${params.lifebitai_gwas_vcf_regenie_1.king_coefficient} \
        $plink_memory \
        --out relatedness
    """
}
...
process infer_ancestry {
    label "tiny_memory"
    publishDir "${params.lifebitai_gwas_vcf_regenie_1.outdir}/king_ancestry_inference/", mode: 'copy'

    input:
    tuple file('keep.tsv'), val(name), file('in.bed'), file('in.bim'), file('in.fam')
    file(ancestry_inference_Rscript)
    set val(ref_name), file('ref.bed.xz'), file('ref.bim.xz'), file('ref.fam.xz')
    ...
    sed -i "s/PREFIX_PLACEHOLDER/subset/g" lifebitai-gwas-vcf-regenie-1-Ancestry_Inference.R

    # calculate PCs
    king -b ref.bed,subset.bed --pca --projection --rplot --prefix subset --cpus ${task.cpus}

    # Run ancestry inference script
    Rscript lifebitai-gwas-vcf-regenie-1-Ancestry_Inference.R subsetpc.txt subset_popref.txt subset
...
process filter_pca {
  tag "${ancestry_group}"
  publishDir "${params.lifebitai_gwas_vcf_regenie_1.outdir}/${ancestry_group}/pca/", mode: 'copy'
  ...
  while [ \$i -le ${params.lifebitai_gwas_vcf_regenie_1.remove_outliers_maxiter} ] && [ \$n_outliers -ne 0 ]; do

    cat plink_remove_ids_*.tsv > all_plink_remove_ids.tsv

    if [ \$(wc -l in.fam | cut -d " " -f1) -gt 5000 ]; then
      plink2 --bfile subset --remove all_plink_remove_ids.tsv --pca ${params.lifebitai_gwas_vcf_regenie_1.number_pcs} approx --out pca_results_\${i} 1>&2
    else
      plink2 --bfile subset --remove all_plink_remove_ids.tsv --pca ${params.lifebitai_gwas_vcf_regenie_1.number_pcs} --out pca_results_\${i} 1>&2
    fi

    lifebitai-gwas-vcf-regenie-1-pca_outliers.R --input=pca_results_\${i}.eigenvec \
      --out=plink_remove_ids_\${i}.tsv \
      --sigma=${params.lifebitai_gwas_vcf_regenie_1.remove_outliers_sigma} > remove_outliers_\${i}.log
...
process transform_phenofile {
  ...
  script:
  """
  lifebitai-gwas-vcf-regenie-1-transform_pheno.R --pheno ${original_pheno} --transform ${transform_tsv} --out_prefix ./
  """
...
process align_pheno_with_test_variant_plink {
  tag "${ancestry_group} ${gwas_tag}"
  publishDir "${params.lifebitai_gwas_vcf_regenie_1.outdir}/${ancestry_group}/${gwas_tag}/", mode: 'copy'
...
process filter_binary_missingness {
    ...
    awk '\$5 < ${params.lifebitai_gwas_vcf_regenie_1.miss_test_p_threshold} {print \$2 }' hwe.missing > hwe.missing_FAIL
...
process regenie_step1_fit_model {
  tag "${ancestry_group} ${gwas_tag}"
  label 'regenie'
  publishDir "${params.lifebitai_gwas_vcf_regenie_1.outdir}/${ancestry_group}/${gwas_tag}/regenie/", mode: 'copy', pattern: "${ancestry_group}-${gwas_tag}-regenie_step1*"
...
process regenie_step2_association_testing {
  tag "${ancestry_group} ${gwas_tag}"
  label 'regenie'
  publishDir "${params.lifebitai_gwas_vcf_regenie_1.outdir}/${ancestry_group}/${gwas_tag}/regenie", mode: 'copy'
  ...
  regenie \
    ...
    --threads ${task.cpus} \
    ${trait_type == "binary" ? '--bt' : ''} \
    --minMAC ${params.lifebitai_gwas_vcf_regenie_1.regenie_min_mac} \
    --minINFO ${params.lifebitai_gwas_vcf_regenie_1.regenie_min_imputation_score} \
    --firth --approx \
    --pThresh 0.01 \
    --pred ${pred} \
...
workflow lifebitai_gwas_vcf_regenie_1{
  take:
    ch_user_input_vcf
  ...
  main:
    ...
    if (params.lifebitai_gwas_vcf_regenie_1.sex_check) {check_sample_sex(merged_plink)}

    ld_prune(merged_plink,
            ch_high_ld_regions)

    if (params.lifebitai_gwas_vcf_regenie_1.remove_related_samples) {
    ...
    if (params.lifebitai_gwas_vcf_regenie_1.run_ancestry_inference) {
      ...
      ch_ancestry_keep_files = infer_ancestry.out.split_ancestry_keep_files_out
                                  ...
                                  .filter{ it[1].readLines().size() >= params.lifebitai_gwas_vcf_regenie_1.min_subpop_size }
    } 
    ...
    if (params.lifebitai_gwas_vcf_regenie_1.run_pca) {
...
workflow{
  ...
  if (params.input_folder_location) {
    ch_user_input_vcf = Channel.fromPath("${params.input_folder_location}/**${params.file_pattern}*.{${params.file_suffix},${params.index_suffix}}")
        ...
        .take( params.lifebitai_gwas_vcf_regenie_1.number_of_files_to_process )
  }
  ...
  ch_ancestry_inference_Rscript = Channel.fromPath("${projectDir}/bin/lifebitai-gwas-vcf-regenie-1-Ancestry_Inference.R", followLinks: false)
  ...
  //ch_lifebitai-gwas-vcf-regenie-1-pca_outliers.Rscript = Channel.fromPath("${projectDir}/bin/lifebitai-gwas-vcf-regenie-1-pca_outliers.R", followLinks: false)
